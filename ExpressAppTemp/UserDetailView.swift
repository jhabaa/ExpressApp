...
struct UserDetailView: View {
    ...
    func printName()->some View{
        Text(user.name)
            ...
            .padding(.top, !(UIDevice().name.contains("iPhone SE") || UIDevice().name.contains("iPhone 8")) ? 50 : 0)
    ...
    @ViewBuilder
    func printAdminDisclaimer()->some View{
    ...
    @ViewBuilder
    func printAdminToAdminDisclaimer()->some View{
        
    }
    @ViewBuilder
    func printUserContatsActions()->some View{
    ...
    @ViewBuilder
    func printSessionButtons()->some View{
            ...
            if !viewThisCommand.isNil{
                ...
                ScrollView(showsIndicators:false){
                    ...
                    if utilisateur.this.isAdmin{
                        switch user.isAdmin {
                        case true:
                            //Admin visit an admin account: Just show name, a disclaimer an a contact
                            printName()
                            printAdminDisclaimer()
                            //MARK: Print contacts if account is different
                            if (utilisateur.this != user){
                                printUserContatsActions()
                            }else{
                                printSessionButtons()
                            }
                            
                        case false:
                            //Admin visit an user account: Show Data's
                            printName()
                            printUserContatsActions()
                            Divider().padding()
                            printCommands()
                            //Delete an account from the admin side
                            Button(action: {
                                Task{
                                    let r:Bool = await utilisateur.delete(user)
                                    if r{
                                        alerte.NewNotification(.amber, "L'utilisateur \(user.surname) \(user.name) est maintenant supprimé", UIImage(systemName: "checkmark.seal.fill")?.withTintColor(.white))
                                    }
                                }
                            }, label: {
